...
const userState = {};
const userBalance = {};
const processedOtps = new Set(); // ডুপ্লিকেট ওটিপি রোধ করার মেমোরি
const MIN_WITHDRAW_AMOUNT = 100.00;
const OTP_REWARD_AMOUNT = 0.70; // প্রতি ওটিপিতে ৭০ পয়সা
...
bot.on('message', async (msg) => {
    ...
    if (text.includes('Support')) {
        ...
        return bot.sendMessage(
            ...
            {
                parse_mode: 'Markdown',
                reply_markup: { inline_keyboard: [[{ text: '👨‍💻 Contact Admin', url: `https://t.me/${username}` }]] }
            }
        );
    }
    ...
    if (userState[chatId] && userState[chatId].step === 'AWAITING_NUMBER') {
        ...
        return bot.sendMessage(chatId, `📲 Number accepted: \`${walletNumber}\`\n\nNow send the *amount of money* you want to withdraw:`, { parse_mode: 'Markdown' });
    }
    ...
    if (userState[chatId] && userState[chatId].step === 'AWAITING_AMOUNT') {
        ...
        return bot.sendMessage(
            ...
            `⚠️ *Confirm Withdraw*\n\n🔹 Method: ${userState[chatId].method}\n📞 Number: \`${userState[chatId].walletNumber}\`\n💰 Amount: \`${amount.toFixed(2)}\` ৳\n\nClick to confirm:`,
            {
                ...
                reply_markup: {
                    inline_keyboard: [
                        [{ text: '✅ Confirm Withdraw', callback_data: 'confirm_withdraw' }, { text: '❌ Cancel', callback_data: 'cancel_withdraw' }]
                    ]
                }
            }
        );
    ...
    // ===============================
    // GET ACTIVE NUMBER (RELIABLE AUTO-FETCH)
    // ===============================
    ...
    if (text.includes('Get Active Number') || text.includes('Refresh Panel')) {
        ...
        try {
            ...
            if (!isJoined) {
                return bot.sendMessage(chatId, `❌ *Please join our channel first.*\n\nChannel: ${config.REQUIRED_CHANNEL}`, { parse_mode: 'Markdown' });
            }

            await bot.sendMessage(chatId, '⏳ Checking live active number from panel...');
            const liveData = await getLiveAccess();

            if (!liveData || !liveData.data || !Array.isArray(liveData.data.services)) {
                return bot.sendMessage(chatId, '❌ No valid data received from the panel.');
            }

            let targetRange = null;
            let selectedServiceName = 'Unknown';

            // সার্ভার থেকে যেকোনো সক্রিয় রেঞ্জ বা সার্ভিস নিখുতভাবে খুঁজে বের করা
            for (const service of liveData.data.services) {
                const ranges = service.ranges || service.range || [];
                if (Array.isArray(ranges) && ranges.length > 0) {
                    const cleaned = String(ranges[0]).replace(/[^0-9]/g, '');
                    if (cleaned) {
                        targetRange = cleaned;
                        selectedServiceName = service.name || service.title || service.app || 'Service';
                        break;
                    }
                }
            }
            ...
            if (!targetRange) {
                return bot.sendMessage(chatId, 'ℹ️ No active range available at the moment.');
            }

            const numResult = await getNewNumber(targetRange);

            if (!numResult || !numResult.data) {
                return bot.sendMessage(chatId, '❌ Failed to allocate number.');
            }
            ...
            const finalAppName = phoneData.service || phoneData.app || selectedServiceName;

            startFastOtpChecker(chatId, phoneNumber);

            await bot.sendMessage(
                chatId,
                `📱 *App / Service:* ${finalAppName}\n` +
                `📍 *Country:* ${countryName}\n` +
                `📞 *Number:* \`${phoneNumber}\`\n\n` +
            ...
            await bot.sendMessage(chatId, '❌ A technical error occurred. Please try again later.');
        } finally {
...
bot.on('callback_query', async (query) => {
    try {
        ...
        if (data && data.startsWith('withdraw_')) {
            ...
            await bot.sendMessage(chatId, `📲 *${method}* selected.\n\nNow send your *${method} number*:`, { parse_mode: 'Markdown' });
        }
...
});st TelegramBot = require('node-telegram-bot-api');
const http = require('http');
...
const userState = {};
const userBalance = {};
const processedOtps = new Set(); // ডুপ্লিকেট বা ভুয়া ওটিপি রোধ করার জন্য
const MIN_WITHDRAW_AMOUNT = 100.00;
const OTP_REWARD_AMOUNT = 0.70; // প্রতিটি ওটিপির জন্য ৭০ পয়সা
...
async function startFastOtpChecker(chatId, phoneNumber) {
    let attempts = 0;
    const maxAttempts = 900; // ১৫ মিনিট পর্যন্ত প্রতি ১ সেকেন্ডে চেক করার জন্য (১৫ মিনিট = ৯০০ সেকেন্ড)
    const startTime = Date.now();
    ...
    const interval = setInterval(async () => {
        ...
        if (attempts > maxAttempts || elapsedTime > maxDurationMs) {
            clearInterval(interval);
            return;
        }
    ...
    try {
        ...
        if (data && data.startsWith('select_app_')) {
            ...
            ranges.forEach((range, rIndex) => {
                const rangeText = typeof range === 'object' ? (range.country || range.name || JSON.stringify(range)) : range;
                rangeKeyboard.push([{
                    text: `🌍 Category / Range: ${rangeText}`,
        ...
        if (data && data.startsWith('getnum_')) {
            ...
            const targetRange = String(typeof rawRange === 'object' ? (rawRange.range || rawRange.code || rawRange) : rawRange).replace(/[^0-9]/g, '');
