...
bot.onText(/^\/start(?:@\w+)?(?:\s+(.+))?$/, async (msg, match) => {
    ...
    // Referral Logic
    if (refId && !isNaN(refId) && Number(refId) !== chatId) {
        const referrerId = Number(refId);
        if (!userData.referredBy) {
            userData.referredBy = referrerId;
            const referrerData = getUserData(referrerId);
            if (!referrerData.referrals.includes(chatId)) {
                referrerData.referrals.push(chatId);
                // Optionally give bonus to referrer here if needed
            }
        }
    }
            ...
            if (otpResult && otpResult.data) {
                ...
                for (let item of items) {
                    ...
                    if (isMatched && messageText) {
                        ...
                        // Check and reward referrer if exists
                        if (userData.referredBy) {
                            const referrerData = getUserData(userData.referredBy);
                            const refBonus = OTP_REWARD_AMOUNT * 0.10; // 10% referral commission example
                            referrerData.totalEarned += refBonus;
                        }
...
bot.on('message', async (msg) => {
    ...
    if (text.includes('REFER & EARN')) {
        ...
        const refText = `🔵 *Refer & Earn Program*\n\n` +
            `Invite your friends and earn rewards when they use the bot!\n\n` +
            `🔗 *Your Referral Link:*\n\`${refLink}\`\n\n` +
            `👥 *Total Referrals:* \`${getUserData(chatId).referrals.length}\``;
        return bot.sendMessage(chatId, refText, { parse_mode: 'Markdown' });
    }

    if (text.includes('LEADERBOARD')) {
        // Simple Leaderboard based on totalEarned
        const sortedUsers = Object.entries(userBalance)
            .sort((a, b) => b[1].totalEarned - a[1].totalEarned)
            .slice(0, 5);

        let lbText = `🏆 *Top Earners Leaderboard*\n\n`;
        if (sortedUsers.length === 0) {
            lbText += `No data available yet.`;
        } else {
            sortedUsers.forEach(([id, data], index) => {
                lbText += `${index + 1}. User \`...${id.slice(-4)}\` - 💵 \`${data.totalEarned.toFixed(2)}\` ৳\n`;
            });
        }
        return bot.sendMessage(chatId, lbText, { parse_mode: 'Markdown' });
    }
    ...
    if (text.includes('WITHDRAW')) {
        ...
        if (currentBalance < MIN_WITHDRAW_AMOUNT) {
            return bot.sendMessage(chatId, `❌ *Insufficient Balance!*\n\nYour current balance is \`${currentBalance.toFixed(2)}\` ৳.\nMinimum withdraw amount is \`${MIN_WITHDRAW_AMOUNT}\` ৳.`, { parse_mode: 'Markdown' });
        }
        userState[chatId] = 'WAITING_FOR_WITHDRAW_METHOD';
        return bot.sendMessage(chatId, `💸 *Withdrawal Menu*\n\nPlease send your payment method and number (e.g., bKash / Nagad: 01XXXXXXXXX):`, { parse_mode: 'Markdown' });
    }

    if (userState[chatId] === 'WAITING_FOR_WITHDRAW_METHOD') {
        const data = getUserData(chatId);
        const currentBalance = data.totalEarned - data.totalWithdrawn;
        
        data.totalWithdrawn += currentBalance;
        delete userState[chatId];

        return bot.sendMessage(chatId, `✅ *Withdrawal Request Submitted!*\n\nAmount: \`${currentBalance.toFixed(2)}\` ৳\nAccount Details: \`${text}\`\n\nAdmin will process your payment within 24 hours.`, { parse_mode: 'Markdown', reply_markup: mainMenu.reply_markup });
    }
    ...
    try {
        ...
        if (data && data.startsWith('num_')) {
            ...
            return bot.editMessageText(
                ...
                `⏰ *Validity:* 15 Minutes`,
                { chat_id: chatId, message_id: messageId, parse_mode: 'Markdown', ...numberKeyword }
            );
        }
    } catch (error) {
